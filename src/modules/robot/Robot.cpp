...
void Robot::load_config()
{
    ...
    for (size_t a = 0; a < MAX_ROBOT_ACTUATORS; a++) {
        ...
        StepperMotor *sm ;
		if(THEKERNEL->config->value(motor_checksums[a][3])->by_default("nc")->as_string() != "nc"
			&& THEKERNEL->config->value(motor_checksums[a][4])->by_default("nc")->as_string() != "nc"
		    && THEKERNEL->config->value(motor_checksums[a][5])->by_default("nc")->as_string() != "nc")
        {
			Pin slavePins[3]; //step_slave, dir_slave, enable_slave
			for (size_t i = 0; i < 3; i++) {
				slavePins[i].from_string(THEKERNEL->config->value(motor_checksums[a][i+3])->by_default("nc")->as_string())->as_output();
			}
        ...
        actuators[a]->change_steps_per_mm(THEKERNEL->config->value(motor_checksums[a][6])->by_default(a == 2 ? 2560.0F : 80.0F)->as_number());
        actuators[a]->set_max_rate(THEKERNEL->config->value(motor_checksums[a][7])->by_default(30000.0F)->as_number()/60.0F); // it is in mm/min and converted to mm/sec
        actuators[a]->set_acceleration(THEKERNEL->config->value(motor_checksums[a][8])->by_default(NAN)->as_number()); // mm/secs²
    }
